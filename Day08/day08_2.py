        ...
        for i, row in enumerate(self.map.positions):
            for j, position in enumerate(row):
                if position.char == self.char and (i,j) != self.position:
                    position.antinode = True
                    row_dist, col_dist = i - self.position[0], j - self.position[1]
                    antinode_row, antinode_col = i + row_dist, j + col_dist
                    while 0<= antinode_row < len(self.map.positions) and 0<= antinode_col < len(self.map.positions[0]):
                        self.map.positions[antinode_row][antinode_col].antinode = True
                        antinode_row += row_dist
                        antinode_col += col_dist
